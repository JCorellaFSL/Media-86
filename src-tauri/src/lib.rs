...
fn upscale_image(directory: String, filename: String, scale: u32) -> Result<String, String> {
    ...
    let original_stem = original_path.file_stem().ok_or("Could not get file stem")?.to_str().ok_or("Invalid file stem")?;
    let extension = original_path.extension().ok_or("Could not get extension")?.to_str().ok_or("Invalid extension")?;
    ...
    let final_filename = new_path.file_name().ok_or("Could not get final filename")?.to_str().ok_or("Invalid final filename")?.to_string();

    // Explicitly convert to RGB8 to ensure maximum compatibility before saving
    upscaled.to_rgb8().save(&new_path).map_err(|e| e.to_string())?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, read_directory, get_image_files, get_full_path, rename_files, rotate_image, upscale_image])
